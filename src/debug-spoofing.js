...
function analyzeSpoofingStatus() {
    ...
    if (hasOperaUA || hasWindowOpera || hasWindowOpr || !isGoogleVendor) {
        ...
        resultElement.innerHTML = '❌ <strong>Spoofing FAILED</strong><br>Opera is still detectable. The Chrome Mask extension may not be working properly.';
    } else if (hasChromeUA && isGoogleVendor && hasWindowChrome) {
        statusElement.className = 'test-result success';
        resultElement.innerHTML = '✅ <strong>Spoofing SUCCESSFUL</strong><br>Browser appears as Chrome. Opera detection is blocked.';
    } else {
        statusElement.className = 'test-result warning';
        resultElement.innerHTML = '⚠️ <strong>Spoofing PARTIAL</strong><br>Some aspects are working but others may need attention.';
    }
}
        ...
        enableBtn.addEventListener('click', async function() {
            try {
                ...
                if (!enabledHostnames.has('localhost')) {
                    ...
                    resultDiv.innerHTML = '<span style="color: green;">✅ localhost enabled! Refreshing page in 2 seconds...</span>';
                    
                    setTimeout(() => {
                        window.location.reload();
                    }, 2000);
                } else {
                    resultDiv.innerHTML = '<span style="color: orange;">⚠️ localhost is already enabled. Try refreshing the page (Ctrl+F5)</span>';
                }
            } catch (error) {
                console.error('Error enabling localhost:', error);
                document.getElementById('enable-result').innerHTML = '<span style="color: red;">❌ Error: ' + error.message + '</span>';
            }
        });
    }
});
