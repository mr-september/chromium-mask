...
if (!chrome || !chrome.runtime) {
    ...
    document.body.innerHTML = '<div class="container"><h1>❌ Extension Context Required</h1><p>This test page must be opened from within the Chrome Mask for Opera extension context. Please install the extension and access this page through the extension.</p></div>';
}
...
async function testServiceWorker() {
    try {
        ...
        if (response && response.success) {
            showResult('sw-result', `✅ Service worker is responding!\n\nMessage: ${response.message}\nTimestamp: ${response.timestamp}\nExtension ID: ${response.extensionId}`, 'success');
        } else {
